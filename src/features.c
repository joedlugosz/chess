...
const char option_controls[N_OPTION_T][10] = {
  "check", "spin", "string", "string", "button", "combo"
};
	  ...
	  case COMBO_OPT:
		{
		  ...
		  for(k = 0; k < opt->combo_vals->n_vals; k++) {
			...
			printf("%s%s %s", ((*((int *)(opt->value)) == k) ? "*" : ""), val->name, (k < opt->combo_vals->n_vals - 1) ? "/// " : "");
		  }
		}
	  case N_OPTION_T:
		break;
...
const feature_s features[N_FEATURES] = {
  { "myname",   TEXT_FEAT, 0, "JoeChess 0.1" },
  { "setboard", INT_FEAT,  0, ""             },
  { "name",     INT_FEAT,  0, ""             },
  { "ping",     INT_FEAT,  0, ""             },
  { "done",     INT_FEAT,  0, ""             },
